...
if ( grep "dotFiles start" ~/.bash_profile -q ); then
	echo "dotFiles found at bashrc"
else
	echo "adding dotFiles to bashrc" && echo '#dotFiles start\nif [ -f ~/.bash_add ]; then\nsource ~/.bash_add\nfi\n#dotFiles end' >> ~/.bash_profile 
fi

if [[ -f ~/.bash_add ]]; then
	echo "bash profile addon already exists"
else
	cp userroot/bash_add ~/.bash_add && echo "Copying bash profile addon"
fi

if [[ -f ~/.vim_add ]]; then
	echo "vim profile addon already exists"
else
	cp userroot/vim_add ~/.vim_add && echo "Copying vim profile addon"
fi

if [[ -f ~/.git-completion.bash ]]; then
	echo "git completion for bash already exists"
else
	cp userroot/git-completion.bash ~/.git-completion.bash && echo "Copying git completion for bash"
fi
